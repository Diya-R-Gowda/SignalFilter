...
## Technical Summary — Week 1–2 + connector reliability work

Covers every 🟢 Done / 🟡 in-progress row in the status table above: the mechanism actually built, the specific decisions made, and the real bugs hit and how each was root-caused and fixed. `⚪ Not started` items aren't summarized here — see their "Week 3+ idea" sections below instead.

**Two-stage ranking pipeline** — `pipeline.py`'s `process_item()` ties both stages together: embed → filter → (if passed) LLM-score → notify → persist. Stage 1 (`embedding.py`) computes cosine similarity between the focus text and item content via `sentence-transformers`/`all-MiniLM-L6-v2`, local and CPU-only. Its threshold was tuned twice against real Slack traffic, both times downward: `0.25 → 0.05` after a live test showed a relevant message scoring only 0.070 while an irrelevant one scored 0.097 (short conversational text doesn't order well against a longer focus string at this model's scale); then `0.05 → 0.0` after a second live test showed a clearly-relevant message ("how is the ui components coming along?") scoring just 0.017 and getting filtered before ever reaching the LLM, while an unrelated message scored negative (-0.035). Final design: stage 1 only blocks actively anti-correlated (negative-similarity) messages and defers everything else to stage 2 — the real judgment call was always meant to live in the LLM, not the embedding filter. Stage 2 (`llm.py`) calls local Ollama (`qwen2.5:3b-instruct`) with a structured-JSON prompt (`{"score": 0-10, "reason": "..."}`), banded scoring guidance, and an explicit rule that automated/bulk/marketing mail always scores 0-1 regardless of keyword overlap with the focus — added after real Gmail marketing mail was scoring higher than it should. `temperature` is pinned to `0` so the same message scores identically across runs. Notify threshold is per-source: Slack fires at `interrupt_score_threshold=4` (casual on-topic check-ins should still notify there), Gmail at a stricter `gmail_interrupt_score_threshold=7` (only clearly actionable/urgent mail), after the user reported Gmail was notifying on too much. **Bug fixed:** `win11toast.toast()` blocks until dismissed; it was firing before the DB commit, so a process restart during an active toast (common while iterating) could lose the record of a real notification. Fixed by committing first and firing the toast on a daemon thread so it can never block or lose pipeline state again.

**Slack connector** (`slack_connector.py`) — Socket Mode, no public URL/ngrok needed. **Bug 1:** `SocketModeHandler.start()` registers a `SIGINT` handler, which only works on the main thread — broke as soon as Slack ran alongside Gmail on a background thread under `--source all`. Fixed with `handler.connect()` + a blocking `threading.Event()` instead, avoiding signal handling entirely. **Bug 2** (more serious, see Graceful connector failure handling below): `App(token=...)` construction itself was happening at module import time in the main thread, and `App.__init__` eagerly calls `auth_test()` — so a dead token crashed the whole process before threading even started, taking Gmail's not-yet-started thread down with it.

**Gmail connector** (`gmail_connector.py`) — OAuth Desktop-app flow (`google-auth-oauthlib`'s `InstalledAppFlow`), token cached to `token.json`, polling `history.list` every 40s against a `historyId` cursor persisted in `SyncState` (avoids re-scanning the whole inbox each cycle). **Bug 1:** `history.list` can fire a `messageAdded` event before the message's subject/snippet are actually indexed, producing empty content that the LLM would hallucinate a plausible-sounding score for — a real false notification. Fixed by skipping empty-content messages and letting Gmail's re-emitted event on the next poll pick them up. **Bug 2** (the serious one): a `print()` of message content crashed with `UnicodeEncodeError` on Windows' default `cp1252` console encoding whenever a message contained emoji/non-ASCII characters — and that crash happened *after* the message's own DB commit but *before* the `historyId` cursor update at the end of the poll, so every subsequent poll restarted from the same stale cursor, reprocessing the same earlier message forever and never reaching newer mail (a real test email silently never showed up). Fixed by forcing UTF-8 stdout at the CLI entry point and wrapping each message's processing in its own try/except, so one bad message can never again block the batch or the cursor advance.

**FastAPI HTTP API** (`main.py` + `schemas.py`) — thin wrappers over existing service functions: `GET`/`POST /focus`, `GET /items` (source/notified filters, pagination capped at 100), `POST /items/{id}/feedback`. **Bug:** CORS was hardcoded to `localhost:5173`, but Vite falls back to the next free port when 5173/5174 are already taken — the dashboard's fetch calls were silently blocked by the browser and misreported as "API unreachable." `curl`-based smoke testing hadn't caught it, since `curl` doesn't enforce CORS. Fixed with `allow_origin_regex=r"http://localhost:\d+"` to match any localhost port.

**React dashboard** (`frontend/`) — Vite + React + TS. Surfaced/filtered columns driven directly by the pipeline's own `notified` flag (no separate UI-side relevance logic), a focus switcher (waits for the `POST /focus` response before updating displayed state, not optimistic), and 👍/👎 feedback buttons. Added a "hide low-relevance items" toggle after real Gmail marketing mail dominated the Filtered column. **Bug:** the first version only checked `llm_score <= 1`, which excluded items with `llm_score === null` — i.e. anything that failed stage 1 and never reached the LLM at all, which was most of the marketing mail. Fixed to treat `!passed_stage1 || (llm_score !== null && llm_score <= 1)` as low-relevance; verified against real DB data (272 of 278 filtered items correctly hidden). Status: type-checks and builds cleanly; not yet manually clicked through in a browser (🟡).

**CLI** (`cli.py`) — `focus` (updatable live from a separate terminal, no listener restart needed) and `run --source {slack,gmail,all}` (one background thread per connector when `all` is selected).

**Connector health visibility** — heartbeats reuse the existing `SyncState` key/value table unmodified (no new model). Gmail writes `gmail_last_poll_at` as the literal first statement of every poll cycle, before any of its branches run, so a heartbeat fires regardless of outcome. Slack (event-driven, no natural polling cadence) gets a synthetic heartbeat via a dedicated 30s daemon thread, started only after `handler.connect()` succeeds. `GET /health` reports staleness as 3× each connector's own interval (Gmail >120s, Slack >90s); a never-written heartbeat counts as stale/unknown, never healthy-by-default. Deliberately out of MVP scope: no escalation/toast-on-stale — a staleness notification risked becoming noisy on its own and undermining the point of the project. Verified live: killed the Gmail listener, confirmed `/health` flipped to `stale:true` only after crossing the 120s threshold, not before.

**Graceful connector failure handling** — root cause was the Slack `App(...)` bug above: a dead token crashed the entire process at import time, in the main thread, before any `threading.Thread` existed — not a thread-isolation failure, a pre-threading one. Fixed by moving `App(...)` construction and event-handler registration out of module scope into `start_slack_listener()` (matching the lazy-construction pattern `gmail_connector.py` already used correctly), wrapped in a try/except that records `slack_last_error_at`/`slack_last_error_message` via `SyncState` and returns without re-raising — no auto-retry, since a dead credential needs a human, not a retry loop. Applied the identical fix symmetrically to `gmail_connector.py`, which had the same latent, previously-unobserved bug. `/health` now forces `stale=true` whenever an error is recorded, independent of heartbeat timing. Dashboard gained a third visual state — amber "crashed" (known cause), distinct from red "stale" (gone quiet, unknown why). Verified live twice: once against the actual dead Slack token already in `.env` (Gmail ran normally under `--source all` while Slack failed and recorded why, process stayed up); once by temporarily hiding Gmail's `credentials.json` (symmetric result). **Follow-up bug:** a resolved error never cleared itself — fixing credentials and restarting left `/health` reporting a healthy connector as still "crashed," since nothing removed the old error record. Fixed with a new `delete_cursor()` helper that removes the `SyncState` row entirely (rather than an empty-string sentinel, since `value` is non-nullable) at the exact point a connection is confirmed established. Verified live by hiding then restoring Gmail's credentials and confirming `/health` returned to a clean `stale:false` state with both error fields `null`.
