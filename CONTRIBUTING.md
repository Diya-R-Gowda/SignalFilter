...
Audited only — no code written yet, per explicit scope. The audit corrected two assumptions the original idea write-up had gotten wrong: the three tunable thresholds (`embedding_threshold`, `interrupt_score_threshold`, `gmail_interrupt_score_threshold`) are read across **two files at three sites**, not one — `pipeline.py:42/44` reads the two score thresholds directly, but `embedding_threshold` is actually read one layer down inside `embedding.py:22`'s `passes_stage1()`; and the plan to mirror how "`.env` seeds focus, then the DB takes over" doesn't apply, because `focus.py` has **no** `.env` seeding mechanism at all — `get_current_focus()` returns `None` with no fallback until a human runs `focus` for the first time. The audit also resolved both of the idea's original open questions rather than leaving them for a build step to re-litigate: validation should reject out-of-range values with a 422 (score thresholds outside `[0, 10]`, matching the real hard clamp already in `llm.py`; embedding threshold outside `[-1, 1]`, cosine similarity's true mathematical range) — specifically because a dashboard invites fast, unreviewed edits in a way a `.env` file + restart doesn't, and a stray typo could silently disable all notifications with no feedback. And `GET /settings` should always return the current effective value (DB override, else the `.env` default), never `null`/"unset", since an un-overridden threshold isn't stale or unknown the way a heartbeat would be — it's a real, currently-effective value with no time-decay risk.
